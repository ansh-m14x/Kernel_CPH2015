...
#ifdef ODM_HQ_EDIT
/*zhouyi@ODM.HQ.BSP.CHG.Basic 2019.11.28 add for thermal */
extern int mtkts_bts_get_hw_temp(void);
#define THERMAL_LED_ON_CONST_INPUT_CUR_MA    500
#define THERMAL_LED_ON_TEMPERATURE    52000
#endif /*ODM_HQ_EDIT*/
...
#ifdef ODM_HQ_EDIT
/*zhouyi@ODM.HQ.BSP.CHG.Basic 2019.11.28 add for thermal*/
	int board_temp = mtkts_bts_get_hw_temp();
#endif /*ODM_HQ_EDIT*/
...
#ifdef ODM_HQ_EDIT
/*zhouyi@ODM.HQ.BSP.CHG.Basic 2019.11.28 add for thermal function*/
		if (board_temp >= THERMAL_LED_ON_TEMPERATURE) {
			if (current_limit > THERMAL_LED_ON_CONST_INPUT_CUR_MA) {
				current_limit = THERMAL_LED_ON_CONST_INPUT_CUR_MA;
			}
		}
#endif /*ODM_HQ_EDIT*/
...
#ifdef ODM_HQ_EDIT
static int boardtemp_limit_current = 0;
#endif
...
#ifdef ODM_HQ_EDIT
  	int board_temp = mtkts_bts_get_hw_temp();
	if( (board_temp >= 52000) && (0 == boardtemp_limit_current) ){
		oppo_chg_set_input_current_limit(chip);
		boardtemp_limit_current = 1;
		charger_xlog_printk(CHG_LOG_CRTI,
			"[Battery] oppo_chg_check_aicl_input_limit boardtemp > 52, limit curretn 500mA \n");
	}else if( (board_temp < 50000) && (1 == boardtemp_limit_current) ){
		oppo_chg_set_input_current_limit(chip);
		boardtemp_limit_current = 0;
		charger_xlog_printk(CHG_LOG_CRTI,
			"[Battery] oppo_chg_check_aicl_input_limit boardtemp < 50, recovery current \n");
	}else{
		charger_xlog_printk(CHG_LOG_CRTI,
			"[Battery] oppo_chg_check_aicl_input_limit  do nothing \n");
	}
#endif
#ifdef CONFIG_OPPO_CHARGER_MTK
